...
internal abstract class SharedContentBaseHandler<TEntity> : SharedHandlerBase<TEntity>
  ...
  protected virtual XElement ConvertPropertyValue( string itemType, string contentType, XElement property, SyncMigrationContext context ) {
    var editorAlias = GetEditorAliasByTypeAndProperty( contentType, property.Name.LocalName, context );
  ...
  private string GetEditorAliasByTypeAndProperty( string contentType, string propertyAlias, SyncMigrationContext context ) {
    string editorAlias = context.ContentTypes.GetEditorAliasByTypeAndProperty( contentType, propertyAlias )?.OriginalEditorAlias ?? string.Empty;
    if ( string.IsNullOrEmpty( editorAlias ) && context.ContentTypes.TryGetCompositionsByAlias( contentType, out IEnumerable<string>? compositionAliases ) && compositionAliases?.Any() == true ) {
      foreach ( var compositionAlias in compositionAliases ) {
        editorAlias = GetEditorAliasByTypeAndProperty( compositionAlias, propertyAlias, context );
